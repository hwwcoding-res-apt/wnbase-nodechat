...
// Join room — only fires once index.html (via the inline script in chat.html)
// has confirmed a live Clerk session and handed us a fresh session token.
// window.__clerkTokenReady is a Promise<string|null> set up in chat.html.
(window.__clerkTokenReady || Promise.resolve(null)).then((token) => {
  if (!token) {
    showNotification("You must sign in first", "error");
    setTimeout(() => {
      location.href = "/";
    }, 1500);
    return;
  ...
  socket.emit("join", { username, room, token }, (error) => {
    if (error) {
      showNotification(error, 'error');
      setTimeout(() => {
        location.href = "/";
